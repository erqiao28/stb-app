			...
			<view class="right-panel-top">
				<scroll-view class="process-list" scroll-y scroll-x>
						...
						>
							...
							<view class="grid-label-cell" style="grid-row: 1; grid-column: 2 / 3">选中</view>
							<view class="grid-label-cell" style="grid-row: 2; grid-column: 2 / 3">顺序</view>
							<view class="grid-label-cell" style="grid-row: 3; grid-column: 2 / 3">工序</view>
							<view class="grid-label-cell" style="grid-row: 4; grid-column: 2 / 3">日产量</view>
							<view class="grid-label-cell" style="grid-row: 5; grid-column: 2 / 3">订单数</view>
							<view class="grid-label-cell" style="grid-row: 6; grid-column: 2 / 3">生产数</view>
							<view class="grid-label-cell" style="grid-row: 7; grid-column: 2 / 3">待派数</view>
							<view class="grid-label-cell" style="grid-row: 8; grid-column: 2 / 3">已完成</view>
							<view class="grid-label-cell" style="grid-row: 9; grid-column: 2 / 3">派工数量</view>
							<view class="grid-label-cell" style="grid-row: 10; grid-column: 2 / 3">员工</view>
							...
							<view class="grid-cell" :class="{ 'selected-column': selectedProcessIds.includes(p.rowid), 'associated-column': p.isAssociated && !selectedProcessIds.includes(p.rowid), 'disabled-column': !selectedProcessIds.includes(p.rowid) && !p.isAssociated }" :style="{ gridRow: 4, gridColumn: 3 + idx }">{{ p.dailyOutput || 0 }}</view>
							...
							<view class="grid-cell" :class="{ 'selected-column': selectedProcessIds.includes(p.rowid), 'associated-column': p.isAssociated && !selectedProcessIds.includes(p.rowid), 'disabled-column': !selectedProcessIds.includes(p.rowid) && !p.isAssociated }" :style="{ gridRow: 6, gridColumn: 3 + idx }">{{ p.allcount || 0 }}</view>
							<view class="grid-cell" :class="{ 'selected-column': selectedProcessIds.includes(p.rowid), 'associated-column': p.isAssociated && !selectedProcessIds.includes(p.rowid), 'disabled-column': !selectedProcessIds.includes(p.rowid) && !p.isAssociated }" :style="{ gridRow: 7, gridColumn: 3 + idx }">{{ p.needCount || 0 }}</view>
							<view class="grid-cell" :class="{ 'selected-column': selectedProcessIds.includes(p.rowid), 'associated-column': p.isAssociated && !selectedProcessIds.includes(p.rowid), 'disabled-column': !selectedProcessIds.includes(p.rowid) && !p.isAssociated }" :style="{ gridRow: 8, gridColumn: 3 + idx }">{{ p.finishCount || 0 }}</view>
							<view class="grid-cell" :class="{ 'selected-column': selectedProcessIds.includes(p.rowid), 'associated-column': p.isAssociated && !selectedProcessIds.includes(p.rowid), 'disabled-column': !selectedProcessIds.includes(p.rowid) && !p.isAssociated }" :style="{ gridRow: 9, gridColumn: 3 + idx }">{{ p.dispatchCount || 0 }}</view>
							<view
		...
		<view class="dispatch-modal-content" @click.stop>
			...
			<view class="dispatch-modal-body">
				...
				<view class="dispatch-grid">
					...
					<!-- 第二行：流转剩余 / 待派数 / 派工数量 / 工时 -->
					<view class="dispatch-grid-cell">
						<text class="grid-cell-label">流转剩余</text>
						<text class="grid-cell-value">{{ dispatchModalFlowRemainCount }}</text>
					</view>
					<view class="dispatch-grid-cell">
						<text class="grid-cell-label">待派数</text>
						<text class="grid-cell-value">{{ dispatchModalNeedCount }}</text>
					</view>
					<view class="dispatch-grid-cell">
...
// 操作后轮询刷新统一配置：
// - 以「墙钟总时长」为硬上限（默认 5 秒），到点立即结束并走兜底刷新（页面有手动刷新按钮）；
// - 轮询间隔 500ms；单次查询在剩余时间预算内执行，避免“次数 × 单次上限”叠加导致总等待失控。
const POLL_INTERVAL = 500
const POLL_MAX_WAIT_MS = 5000
// 轮询内单次查询硬超时（毫秒）：网络差时避免单次请求挂起时间过长（实际以剩余预算为准，取两者较小值）
const POLL_QUERY_TIMEOUT = 3000

/**
 * Promise 硬超时护栏：race 目标 promise 与定时 reject，保证最多占用 ms 毫秒。
 * 网络差/请求挂起时及时掐断等待（请求在后台失败不影响界面），避免单轮查询被拖长。
 */
...
/**
 * 统一轮询执行器：整体最多等待 maxWaitMs（墙钟），期间反复执行 query 直到 isDone 命中。
 * - 单次查询超时按「剩余预算」收缩，保证总时长不会因查询慢而被拉长；
 * - 上一轮查询超时后不并发发起新请求，而是复用并等待其在途 Promise 收尾，
 *   配合 loadProducts 的版本号守卫，杜绝“旧请求晚到覆盖新数据”的竞态。
 * @param {Function} query 每轮执行的数据查询，返回 Promise
 * @param {Function} isDone 查询完成后判断是否命中（入参为最近一轮查询结果），返回 true 立即结束
 * @param {number} [maxWaitMs] 可选，覆盖默认的 5 秒墙钟上限
 * @returns {Promise<boolean>} 是否在时限内命中
 */
const pollUntil = async ({ query, isDone, maxWaitMs = POLL_MAX_WAIT_MS }) => {
	const deadline = Date.now() + maxWaitMs
	let inFlight = null
	// 在途查询自身是否失败（区别于“单轮超时未归”）：失败则丢弃在途、下一轮重新发起，
	// 避免对已失败的 Promise 反复 await、空转到超时也不再发起新查询
	...
	while (Date.now() < deadline) {
		...
		try {
			if (!inFlight) {
				inFlight = Promise.resolve(query())
				inFlightFailed = false
				// 永久挂接失败标记：兼防期间无人 await 时产生 unhandled rejection
				inFlight.catch(() => { inFlightFailed = true })
			}
			const result = await withTimeout(inFlight, Math.min(POLL_QUERY_TIMEOUT, budget))
			inFlight = null
			if (isDone(result)) {
				return true
			}
		} catch (e) {
			if (inFlight && inFlightFailed) {
				// 查询自身失败：丢弃该在途 Promise，下一轮重新发起查询
				inFlight = null
				console.warn(`轮询查询失败（第${round}轮），下一轮重新发起:`, e)
			} else if (inFlight) {
				// 单轮查询未在时限内返回：保留在途 Promise 供下一轮复用等待，避免并发请求竞态
				console.warn(`轮询查询未在时限内返回（第${round}轮），等待在途请求收尾:`, e)
			}
		}
		const remain = deadline - Date.now()
		if (remain > 0) {
			await new Promise((resolve) => setTimeout(resolve, Math.min(POLL_INTERVAL, remain)))
		}
	}
	return false
}
...
// 待派数：勾选工序的待派数（needCount）平均值，弹窗内代替原"小时产量"展示
const dispatchModalNeedCount = ref(0)
...
// 派工设置弹窗：勾选工序的平均小时产量（不再在弹窗展示，仅用于下方工时计算）
...
const PROCESS_DETAIL_FIELD_MAP = {
	...
	// 待派数字段：工序列表"待派数"列与派工设置弹窗展示均取该字段（区别于流转剩余）
...
const loadAddProductList = async () => {
	...
	try {
		...
		// 统一在 finally 中收口 loading 与 loading 标志位，避免成功路径中途 hide 后又触发重复 hide。
...
const addProductsByRowids = async (rowids, productionCodes = []) => {
	...
	try {
		...
		// 添加成功后轮询等待新产品数据写入完成再刷新渲染（墙钟最多 5 秒，命中即停）
		// 复用上方"添加中..."的 loading，不再重复 showLoading（App 端 show/hide 计数式配对，多 show 一次会导致转圈无法关闭）
		// 记录添加前的产品条数，供“无生产编号”场景退化为条数增加判断
		const initialProductCount = productList.value.length
		const found = await pollUntil({
			query: () => loadProducts(true, true),
			isDone: () => {
				...
				if (expectedCodes.length > 0) {
					return expectedCodes.every(code => productList.value.some(p => p.productionCode === code))
				}
				return productList.value.length > initialProductCount
			}
		})
...
const handleProcessListConfirm = async (productRowid) => {
	...
	// 只有已设置且大于 0 的有效数值才传递用户输入；未设置/已被清除时按默认值口径计算
	...
	// 默认值口径：先取勾选工序的流转剩余平均，流转剩余为 0/空 时再取待派数平均
	...
	if (hasUserInput) {
		...
		// 流转剩余与待派数均无有效值：没有可默认派工的数量，禁止提交（需先到派工设置自定义数量）
		if (dispatchCount <= 0) {
			uni.showToast({ title: '流转剩余和待派数均无有效数量，请到派工设置填写派工数量', icon: 'none' })
			return
		}
	}
	...
	try {
		...
		if (!isSameDate) {
			// 产品将迁移到派工日期，等其从当前筛选日期列表消失：等待时长与确认派工策略一致，
			// 基础 3 秒，每勾选 1 条工序增加 0.5 秒，墙钟最多 5 秒（pollUntil 内部统一收口）
			const waitMs = Math.min(3000 + checkedProcesses.length * 500, 5000)
			const removed = await pollUntil({
				query: () => loadProducts(true, true),
				maxWaitMs: waitMs,
				isDone: () => !productList.value.some(p => p.uniqueKey === productRowid)
			})
			if (!removed) {
...
const doConfirmDispatch = async () => {
	...
	try {
		...
		// 轮询等待预派工条数匹配后刷新（墙钟最多 waitMs，命中即停）
		uni.showLoading({ title: '刷新中...', mask: true })
		const found = await pollUntil({
			query: () => loadProducts(true, true),
			maxWaitMs: waitMs,
			isDone: () => {
				const currentCount = productList.value.reduce(
					(sum, item) => sum + (item.preDispatchRowids?.length || 0),
					0
				)
				return currentCount === expectedPreDispatchCount
			}
		})
...
// 产品列表加载版本号：后发起的加载拥有更高版本，先发起但晚返回的结果直接丢弃，
// 防止轮询/下拉刷新/onShow 等并发刷新时“旧请求晚到”覆盖新数据
let loadProductsSeq = 0
const loadProducts = async (reset = true, forceSilent = false) => {
	const seq = ++loadProductsSeq
	// 静默刷新（轮询/兜底）不切换 loadingProducts，避免左侧空态占位「暂无产品/暂无工序」在轮询期间反复闪烁；
	// 顶层 loading 由调用方的外层 uni.showLoading 提供。
	if (!forceSilent) {
	...
	try {
		...
		// 记录刷新前的订单集合与收起状态：本次刷新仅让新出现的订单默认收起，
		// 其余保留用户刷新前展开/收起状态，避免轮询静默刷新时左侧列表整体跳动闪烁
		const prevOrderNos = new Set(productList.value.map((p) => p.orderNo || '未分类'))
		const prevCollapsedOrders = new Set(collapsedOrderIds.value)

		// 版本号守卫：期间已有更新的加载发起，本次结果已过期，直接丢弃避免旧数据覆盖
		if (seq !== loadProductsSeq) {
			return
		}
		...
		// 产品列表默认收起：新出现的订单默认收起，其余订单沿用用户当前的展开/收起状态
		const currentOrderNos = [...new Set(productList.value.map((p) => p.orderNo || '未分类'))]
		collapsedOrderIds.value = currentOrderNos.filter((orderNo) => {
			if (prevOrderNos.has(orderNo)) return prevCollapsedOrders.has(orderNo)
			return true
		})
		...
		// 只有最新一次加载才负责复位 loading，避免旧请求晚返回提前关掉新请求的 loading。
		// 静默刷新不接管 loading 状态，防止轮询期间外层 loading 被人为关掉。
		if (seq === loadProductsSeq && !forceSilent) {
...
// 用于派工设置弹窗展示字段（生产数量/前道流转/流转剩余/待派数/完成数量），并作为派工数量默认值口径（流转剩余优先，待派数次之）
...
const openDispatchModal = async (product) => {
	...
	// 新增展示字段：前道流转 / 流转剩余 / 待派数 / 生产数量 = 勾选工序的平均值（跳过0）
	...
	dispatchModalNeedCount.value = needCountAvg
...
const saveDispatchModal = () => {
	...
	// 提交时按默认口径计算（先取流转剩余平均，流转剩余为 0/空 时再取待派数平均，与工序列表"确定"一致），日期仍按所选日期派工
	if (val === '' || num === 0) {
...
const waitForPreDispatchDailyWage = async (product, checkedProcessRowids, targetDispatchCount) => {
	...
	return pollUntil({
		query: () => loadAssociatedProcessDetails(product),
		isDone: (associatedMap = new Map()) => {
			return checkedProcessRowids.every(rowid => {
...
const waitForPreDispatchEmployeeUpdate = async (preDispatchRowid, targetEmployeeIds) => {
	return pollUntil({
		...
		isDone: (res) => {
			...
			return targetSet.size === sidSet.size && [...targetSet].every(id => sidSet.has(id))
		}
	})
}
	...
	try {
		...
		const newProcesses = rows.map((item) => {
			...
			return {
				...
				// 前道流转 / 流转剩余：派工设置弹窗展示与派工默认值均以流转剩余为准
...
const getEmployeeCellStyle = (processes, idx) => {
	...
	if (span <= 1) return { gridRow: 10, gridColumn: 3 + idx }
	return { gridRow: 10, gridColumn: (3 + idx) + ' / span ' + span }
}
...
// 判断"确定"是否可点击：已勾选工序即可（不再校验待派数/流转剩余，允许任意数量派工）
const isDispatchConfirmEnabled = (productRowid) => {
	...
	return checkedProcesses.length > 0
}
...
const waitForProcessUpdate = async (product, options = {}) => {
	...
	// 轮询重载该产品工序数据（墙钟最多 5 秒，命中即停）。
	// 注意：每轮不能预先清空 processList —— loadProductProcesses 拿到最新数据后才一次性替换渲染数组，
	// 轮询期间表格始终有旧数据兜底渲染，避免工序列表出现"清空→重填"的闪烁
	return pollUntil({
		query: async () => {
			loadedProductIds.value = loadedProductIds.value.filter(id => id !== productRowid)
			await loadProductProcesses(product)
		...
		isDone: () => {
			...
			return countChanged || customChanged
		}
	})
}
	...
	.main-content {
		...
		.right-panel {
			...
			.process-list {
				...
				// 单个产品的整块：顶栏（订单编号+产品名称）独立一行，下方才是网格
				.process-table-wrap {
					width: fit-content;
					margin: px2vw(10px);
					border: 1px solid #999;

					// 顶部订单编号 + 产品名称栏：独立块级元素，宽度随内容自适应，
					// 不再横跨网格、不撑开列轨道，无工序时按钮列与标签列依旧并列靠左
					.grid-order-bar {
